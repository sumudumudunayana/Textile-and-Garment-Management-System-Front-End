...
export class ReadyMadeOrderAddComponent implements OnInit {
  ...
  fetchProducts(): void {
    this.products = [
      ...
      { id: 11, name: 'Bath Robe', price: 2250, imageUrl: '/bathrobr.webp' },
      { id: 12, name: 'Bottom', price: 1500, imageUrl: '/bottom.jpg' },
      { id: 13, name: 'Hoodie', price: 3250, imageUrl: '/hoodie.webp' },
      { id: 14, name: 'Sarong', price: 1200, imageUrl: '/sarong.webp' },
      { id: 15, name: 'Shirt', price: 2650, imageUrl: '/shirt.webp' },
    ];
  ...
  dec(p: Product): void {
    this.qtyMap[p.id] = Math.max(1, (this.qtyMap[p.id] ?? 1) - 1);
  }
  ...
  addToCart(p: Product): void {
    ...
    if (idx > -1) {
      items[idx].qty += qty;
      items[idx].lineTotal = this.round2(
        items[idx].qty * items[idx].product.price
      );
    } else {
